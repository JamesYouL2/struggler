...
class StrategicWeights:
    ...
    # THE PLANNER'S TIE-BREAK BAND, in Ops. At 0 (as shipped) the planner
    # is not consulted. Set, the whole-hand assignment's strict preferences
    # reorder only the card plays the scorer already rates within this many
    # Ops of its best -- the maintainer's 2026-09-25 shape ("only when the
    # normal decision is tied or close to tied"), after every variant that
    # let the plan override the scorer outright lost. `_plan_band`.
...
#   hand_assignment  the turn-assignment planner's band, in Ops (step 3
#                of the hand planner plan): at 0 every card is priced and
#                chosen per card as it always has been; at a positive
#                value `hand_plan` allocates the whole hand and its strict
#                preferences break ties among the card plays the scorer
#                rates within this many Ops of its best (`_plan_band`)
#
...
class StrategicPlayer:
    ...
    def rank_actions(self, observation: Observation):
        ...
        try:
            keyed = [(self.safety_key(observation, a), a) for a in decision.options]
            # THE PLANNER BREAKS NEAR-TIES, AND NOTHING ELSE. `pref` demotes
            # only what the assignment STRICTLY prefers not to play now
            # (never an order, never a tie: see `_plan_lead`), and it counts
            # only for options inside `_plan_band` -- the ones the scorer
            # already rates within `hand_assignment` Ops of its best. Sorted
            # above the whole risk/score blend, as it was until 2026-09-25,
            # demoting a card promoted the next one whatever it cost: every
            # planner variant measured lost (strict lead -0.061, headline
            # only -0.005) with USSR-seat nuclear losses up 7 -> 26 and
            # 25 -> 55 (docs/notes/claude/2026-09-23-the-planner-plays-its-
            # hand-in-alphabetical-order.md). Within the band the plan
            # arbitrates what the scorer cannot tell apart; outside it the
            # scorer's order stands. At 0 the band is empty and `pref` is
            # never asked, so the shipped ordering is unchanged -- the
            # parity corpus pins that.
            band = self._plan_band(observation, keyed)
            return sorted(
                keyed,
                key=lambda pair: (pair[0][0], self._plan_pref(observation, pair[1])
                                  if id(pair[1]) in band else 0)
                + pair[0][1:],
                reverse=True)
        finally:
    ...
    def _after_reply(self, obs: Observation, cid: str, points: int,
        ...
        **uncontrolled** one costs one, so **a break that does not also
        ...
        The answer must also be legal: the opponent needs a later Action
    ...
    def _may_place(self, obs: Observation, side: Side, cid: str, when: int) -> bool:
        """Whether `side` can place in `cid` on its next Action Round."""
        board = self.board
    ...
    def _plan_band(self, obs: Observation, keyed) -> frozenset:
        """The options the planner may reorder: card plays whose safety key
        matches the best one's `certain` and risk slots and whose score is
        within `hand_assignment` Ops (at this turn's `ops_value`) of it.

        Empty -- so `_plan_lead`'s re-solves never run -- when the planner
        is off, the decision is not a card play, or fewer than two options
        are close enough for a tie-break to matter. The planner arms were
        slow (single games past the 1200 s stall floor) because the lead
        re-solved the hand on every ranked decision; most decisions have a
        clear best and now ask it nothing.
        """
        width = self.weights.hand_assignment
        ...
        return band if len(band) > 1 else frozenset()
