...
- **M2**: `delta` multiplied the regional VP change by the changed
  ...
  from Pakistan. Variant a: the regional VP carries no urgency on any path.
"""
...
def test_the_regional_term_carries_no_urgency_on_any_path():
    """Variant a, pinned where Codex measured it. US +2 Iran on an empty
    turn-1 board, access off: `delta` returned 52.9822222222 and the board
    moved 47.6666666667, the gap `1.3 * 4 VP * (2.0222 - 1)` -- Iran's
    urgency applied to the Middle East's presence VP by `delta` alone. The
    board's answer is unchanged by this variant; `delta` now gives it."""
    engine = _empty_engine()
    obs = engine.observe(Side.US)
    bot = _no_access()
    bot.prepare(obs)
    expected = _board_difference(bot, Side.US, 'Iran', 2, 0)
    assert expected == pytest.approx(47.6666666667, abs=1e-9)
    assert bot.delta(obs, 'Iran', own=2) == pytest.approx(expected, rel=0, abs=1e-9)
    _total, board = _placements_in_order(_no_access, [('Thailand', 2), ('Pakistan', 2)])
    assert board == pytest.approx(109.2502222222, abs=1e-9)
