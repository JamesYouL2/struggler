...
- [The fresh block answers it: +0.054 [+0.031, +0.078], and `battleground` goes](2026-09-21-the-fresh-block-answers-the-fit.md) — 2026-09-21 (the pre-registered bound cleared on an independent block, so the guessed country tiers and `country_value`'s un-fitted half are deleted; the -0.001 on the old block was the block, not the weights; and the +0.054 is partly the base arm falling, not all gain)
