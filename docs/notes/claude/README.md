...
- [Deleting `scoring_rival` and `coup_discount`: the rule, before the number](2026-09-26-deleting-scoring-rival-and-coup-discount.md) — 2026-09-26
...
- [The assignment planner plays its hand in alphabetical order](2026-09-23-the-planner-plays-its-hand-in-alphabetical-order.md) — 2026-09-23 (run 35814771005's -0.240 is a tie-break, not a verdict on allocation: flat per-round prices leave the DP no reason to order cards, `offer` breaks the tie by key, and `_plan_pref` leads with `rounds[0]`; in four played games the lead overrode the scorer in 168/207 action rounds, 152 of them alphabetical-first. The China Card is also invisible to the plan. And hold == play at hold_option 0, so WHICH cards are held is a tie-break too; the fix on fix/hand-planner-lead leads only on strict preferences. READ: strict lead -0.061 [-0.090, -0.032], hold-the-worst -0.088 [-0.117, -0.060], both stopped at the halfway check; the gate stays shut, the loss is the USSR seat and nuclear losses rise. Headline-only, after the opponent-card fix: -0.005 [-0.023, +0.014] over 1148 seeds, gate shut, USSR nuclear losses 25 -> 55)
...
- [The fit clears on a fresh block: +0.054 [+0.031, +0.078]](2026-09-21-the-fit-clears-on-a-fresh-block.md) — 2026-09-21 (the pre-registered line is cleared on 1020 paired seeds; the base arm is measurably WORSE than bc5ef93 and the fitted one measurably better; the two blocks are 1.5 SE apart and pool to +0.038 [+0.022, +0.055]; step 4 is unblocked but not taken)
- [Handoff, 2026-09-21: what landed, what is in flight, and what to tell the next model](2026-09-21-handoff-and-what-to-hand-a-weaker-model.md) — 2026-09-21 (the session's own mistakes, ordered by damage: profile to find and time unprofiled to price, float regrouping made twice, a guard in a path that never ran, and a pre-registered rule that read -0.001 and was not moved)
...
- [Cheaper readings: a shard cache, sequential waves, and 8% off the bot](2026-09-21-cheaper-readings.md) — 2026-09-21 (a re-dispatched shard becomes free; a decided arm stops at half its seeds for 2% of the final interval; exact speedups worth 5.9% + 2.1%, the 42% of neighbour sweeps that repeat, and the float-regrouping that nearly went in twice)
- [The access scale leak cost nothing, and the prediction was wrong](2026-09-21-the-leak-cost-nothing.md) — 2026-09-21 (+0.011 [-0.007, +0.029]: the leak is worth nothing and the preceding note is RETRACTED. Its own block-variance claim is corrected in place: the two gaps are 1.7 and 1.2 SE, which is not a demonstration)
...
- [Don't revert to v0.2.1: it is not the simpler bot](2026-09-21-do-not-revert-to-v0.2.1.md) — 2026-09-21 (level in strength, but the board-value layer is 19 live weights against HEAD's 10; the codebase grew 66% in instruments, not knobs; simplify forward, not backward)
...
- [The reply budget: the opponent answers with their best card](2026-09-19-reply-budget-best-card.md) — 2026-09-19 (why v0.2.1 still wins: the removed China phantom; reply = 4 and a best-card model, anchored arms)
