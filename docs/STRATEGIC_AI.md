...
- The one-ply reply look-ahead only discounts a placement when the opponent
  ...
  next payout and no reply is charged. With each budget the opponent takes
...
## Fitted country weights (on since 2026-09-18)
...
`matched_scale`; half and double both gated worse. `country_value` then prices our
